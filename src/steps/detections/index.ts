...
export async function fetchDetections(
  ...
  await client.iterateDetections(async (detection) => {
    const detectionEntity = await jobState.addEntity(
      createDetectionEntity(detection),
    );

    await client.iterateDetectionInstances(
      detection.avd_id,
      async (instance) => {
        const repository = await jobState.findEntity(
          buildRepositoryEntityKey(instance.repository_id),
        );
        if (!repository) {
          return;
        }
