...
export class AquaSecClient {
  ...
  private async isBearerTokenExpired(): Promise<boolean> {
    const response = await fetch(
      `${this.endpoint}/v2/accounts/${this.accountId}`,
      {
        method: 'GET',
        headers: {
  ...
  private async bearerTokenRequest<T>(
    path: string,
    opts?: {
      page: number;
    },
    ...
    const pageQuery = opts?.page ? `?page=${opts.page}` : '';
    const response = await fetch(`https://api.aquasec.com${path}${pageQuery}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${this.bearerToken}`,
        'Content-Type': 'application/json',
  ...
  private async paginatedRequest<S, T extends AquaSecPage<S>>(
    path: string,
  ...
  async iterateRepositories(
    iteratee: ResourceIteratee<AquaSecRepository>,
  ): Promise<void> {
    await this.paginatedRequest(
      '/v2/build/repositories',
      async (res: AquaSecRepositoriesResponse) => {
  ...
  async iterateDetections(
    iteratee: ResourceIteratee<AquaSecDetection>,
  ): Promise<void> {
    await this.paginatedRequest(
      '/v2/build/detections',
      async (res: AquaSecDetectionsResponse) => {
  ...
  async iterateDetectionInstances(
    avdId: string,
    iteratee: ResourceIteratee<AquaSecDetectionInstance>,
  ): Promise<void> {
    await this.paginatedRequest(
      `/v2/build/detections/${avdId}/instances`,
      async (res: AquaSecDetectionInstancesResponse) => {
